...
//for (j=NYK2; j<NY; j++)
//{ T[j][NX-1] = T2; TT[j][NX-1] = T2; }
ofstream fout("T1.dat",ios_base::out | ios_base::trunc |
ios_base::binary);
for (j = 0; j < NY; j++) {
for (i = 0; i < NX; i++) {
PRES w = T[j][i];
...
if (i == i1 && j == j1)
	TT[j][i] = t0 - bet_3*(alf_4*T[j-1][i] + alf_2*T[j][i-1] + alf_3*T[j][i+1] + alf_1*T[j+1][i] + gam_2*t0);
if (i == i1 && j == j2)
	TT[j][i] = t0 - bet_3*(alf_1*T[j-1][i] + alf_2*T[j][i-1] + alf_3*T[j][i+1] + alf_4*T[j+1][i] + gam_2*t0);
if (i == i2 && j == j1)
	TT[j][i] = t0 - bet_3*(alf_4*T[j-1][i] + alf_3*T[j][i-1] + alf_2*T[j][i+1] + alf_1*T[j+1][i] + gam_2*t0);
if (i == i2 && j == j2)
	TT[j][i] = t0 - bet_3*(alf_1*T[j-1][i] + alf_3*T[j][i-1] + alf_2*T[j][i+1] + alf_4*T[j+1][i] + gam_2*t0);

if (i>0 && i<i1 && j>0 && j<NY-1)
	TT[j][i] = t0 - bet_1*(alf_1*(T[j][i-1]+T[j][i+1]) + alf_2*(T[j-1][i]+T[j+1][i]) + gam_1*t0);
if (i>i2 && i < i3 && j >0 && j < NY-1)
	TT[j][i] = t0 - bet_1*(alf_1*(T[j][i-1]+T[j][i+1]) + alf_2*(T[j-1][i]+T[j+1][i]) + gam_1*t0);
